...
class SQLNetCondPredictor_cond(nn.Module):
    def __init__(self, N_word, N_h, N_depth, max_col_num, max_tok_num, use_ca, gpu):
        super(SQLNetCondPredictor_cond, self).__init__()
        self.N_h = N_h
        ...
        self.cond_num_lstm = nn.LSTM(input_size=N_word, hidden_size=int(N_h/2),
                ...
                dropout=0.3, bidirectional=True)#1
        
        self.cond_num_att = nn.Linear(N_h, 1)#1
        
        
        self.cond_num_out = nn.Sequential(nn.Linear(N_h, N_h),
                nn.Tanh(), nn.Linear(N_h, 5))#1
        ...
        self.cond_num_col_att = nn.Linear(N_h, 1)#1
        
        self.cond_num_col2hid1 = nn.Linear(N_h, 2*N_h)#1
        
        self.cond_num_col2hid2 = nn.Linear(N_h, 2*N_h)#1
        ...
        if use_ca:
            ...
            self.cond_col_att = nn.Linear(N_h, N_h)#11
        else:
        ...
        self.cond_col_out_K = nn.Linear(N_h, N_h)#11
        self.cond_col_out_col = nn.Linear(N_h, N_h)#11
        self.cond_col_out = nn.Sequential(nn.ReLU(), nn.Linear(N_h, 1))#11

        self.cond_op_lstm = nn.LSTM(input_size=N_word, hidden_size=int(N_h/2),
                num_layers=N_depth, batch_first=True,
                dropout=0.3, bidirectional=True)###1
        if use_ca:
            self.cond_op_att = nn.Linear(N_h, N_h)#111
        else:
            self.cond_op_att = nn.Linear(N_h, 1)
            
            
        self.cond_op_out_K = nn.Linear(N_h, N_h)#111
        
        self.cond_op_name_enc = nn.LSTM(input_size=N_word, hidden_size=int(N_h/2),
                num_layers=N_depth, batch_first=True,
                dropout=0.3, bidirectional=True)#111
