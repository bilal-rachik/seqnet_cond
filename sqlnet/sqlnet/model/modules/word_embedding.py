    ...
    def gen_x_batch(self, q, col):
        ...
        for i, (one_q, one_col) in enumerate(zip(q, col)):          
            ...
            if self.our_model:
                if self.trainable:
                    a=list(q_val)
                    val_embs.append([1] + a + [2])  #<BEG> and <END>
                else:
                    a=list(q_val)
                    val_embs.append([np.zeros(self.N_word, dtype=np.float32)] + a + [np.zeros(self.N_word, dtype=np.float32)])  #<BEG> and <END>
                val_len[i] = 1 + len(a) + 1
            else:
                ...
                if self.trainable:
                    ...
                    b=list(col_val)
                    a=list(q_val)
                    val_embs.append( [0 for _ in self.SQL_TOK] + b + [0] + a + [0])
                else:
                    col_val = map(lambda x:self.word_emb.get(x, np.zeros(self.N_word, dtype=np.float32)), one_col_all)
                    b=list(col_val)
                    a=a=list(q_val)
                    val_embs.append( [np.zeros(self.N_word, dtype=np.float32) for _ in self.SQL_TOK] + b + [np.zeros(self.N_word, dtype=np.float32)] + a + [np.zeros(self.N_word, dtype=np.float32)])
                val_len[i] = len(self.SQL_TOK) + len(b) + 1 + len(a) + 1
        max_len = max(val_len)
